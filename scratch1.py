...
paths = [file for file in glob.glob(r"/Users/teehe/Desktop/DEV/**/*json")]
my_file = Path(r"/Users/teehe/Desktop/inverted-index-creator/inverted_index_first_10.txt")
my_dup = Path(r"/Users/teehe/Desktop/inverted-index-creator/duplicate.txt")
    ...
    def response():
        ...
        if run_type =='s' and my_file.is_file():
            ...
            def queryresponse():
                ...
                string_display = str(total_time)
                L3 = Label(new_wind, text="total time is " + string_display)
                L3.place(relx=0.8, rely=0.7, anchor=CENTER)

            new_wind = Tk(className=' Search')
            new_wind.geometry("500x400")
            ...
            E2['font'] = fontSize
            butt = Button(new_wind, text='Enter', bg='#0052cc', command=queryresponse)
            butt.place(relx=0.8, rely=0.5, anchor=CENTER)
            butt['font'] = fontSize
            butt.bind()
            ...
            for i in range(len(group_index)):
                ...
                end_time = time.time()
                L2 = Label(top)
                string_display = "indexed files "+str(num_of_paths-group_index[i])+" through "+str(num_of_paths)+" in "+str(end_time-start_time)+" seconds"
                L2["text"] = string_display
                L2.place(relx=0.8, rely=0.5, anchor=CENTER)
